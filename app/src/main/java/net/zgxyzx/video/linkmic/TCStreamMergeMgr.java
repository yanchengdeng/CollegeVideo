package net.zgxyzx.video.linkmic;
...
public class TCStreamMergeMgr {
            ...
            public void run() {
                ...
                for (int retryIndex = retryCount; retryIndex >= 1; --retryIndex) {
                    try {
                        ...
                        req.put("Action", "MergeVideoStream");
                        req.put("userid", TCLoginMgr.getInstance().getLastUserInfo().identifier);
                        ...
                        //初始化https请求
                        URL url = new URL(TCConstants.SVR_POST_URL);
                        ...
                        JSONObject retData = rsp.optJSONObject(TCConstants.SVR_RETURN_DATA);
    ...
    private JSONObject createRequestParam() {
        ...
        try {
            ...
            para.put("app_id", TCConstants.XIAOZHIBO_APPID);
